...
func flattenServiceSpec(in v1.ServiceSpec) []interface{} {
	...
	if len(in.Ports) > 0 {
		att["port"] = schema.NewSet(schema.HashResource(servicePortResource), flattenServicePort(in.Ports))
	}
...
func expandServiceSpec(l []interface{}) v1.ServiceSpec {
	...
	obj := v1.ServiceSpec{}
	log.Printf("Expanding service spec: %#v", l)
	if v, ok := in["port"].(*schema.Set); ok && v.Len() > 0 {
...
func patchServiceSpec(keyPrefix, pathPrefix string, d *schema.ResourceData) PatchOperations {
	...
	if d.HasChange(keyPrefix + "session_affinity") {
		ops = append(ops, &ReplaceOperation{
			...
			Value: d.Get(keyPrefix + "session_affinity").(v1.ServiceAffinity),
		})
	}
